<?php
	...
	function simpan($table, array $field, $redirect){
		...
		if($jalan){
			echo "<script>alert('Berhasil Input Data');document.location.href='$redirect'</script>";
		} else {
			echo mysql_error();
		}
	}
	...
	function tampiljurusan($tablejurusan){
		@$sql = "SELECT * FROM $table";
		@$tampil = mysql_query($sql);
		while($data = mysql_fetch_array($tampil))
			@$isi[] = @$data;
	...
	function hapus($table, $where, $redirect){
		...
		@$jalan = mysql_query($sql);
		if(@$jalan){
			echo "<script>alert('Berhasil Menghapus Data');document.location.href='$redirect'</script>";
		}else{
			echo mysql_error();
		}
	}
	...
	function ubah($table, array $field, $where, $redirect){
		...
		@$jalan = mysql_query($sql);
		if($jalan){
			echo "<script>alert('Berhasil Mengedit data');document.location.href='$redirect'</script>";
		}else{
			echo mysql_error();
		}
	}
